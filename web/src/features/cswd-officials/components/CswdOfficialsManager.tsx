...
import { CswdOfficialsTable } from './CswdOfficialsTable';
...
export function CswdOfficialsManager() {
  const [officials, setOfficials] = useState<CswdOfficial[]>([]);
  const [loadingOfficials, setLoadingOfficials] = useState(false);

  // Dialog states
  const [addDialogOpen, setAddDialogOpen] = useState(false);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [editingOfficial, setEditingOfficial] = useState<CswdOfficial | null>(null);
  ...
  const fetchOfficials = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/cswd-officials`);
  ...
  const handleEditOfficial = async (values: OfficialFormValues) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`/api/cswd-officials/${editingOfficial.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(values)
      });
  ...
  const handleToggleStatus = (official: CswdOfficial) => {
    setConfirmDialog({
      open: true,
      official,
  ...
  const handleDelete = (official: CswdOfficial) => {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (mode === 'delete') {
        const res = await fetch(`/api/cswd-officials/${official.id}`, {
          method: 'DELETE'
        });
        if (!res.ok) {
          const data = await res.json();
  ...
  const openEditDialog = (official: CswdOfficial) => {
